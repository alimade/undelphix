...
// Muutetaan myöhemmin siten ett?pallon
// annetaan menn?syvemmälle pussiin ennenkuin
// se poistetaan pelipöydält?
// Näin saadaan peli mallinnettua enemmän
// oikean biljardin mukaiseksi.
// Lisätääm myös pallojen määr?viiteentoista
// sek?kahta väri?+ musta pallo eli kasipallo.
...
const
  InitialNumBalls = 6; // Pallojen maksimilukumäär?1 alussa
  ...
  YLAREUNA = 40;       // kun pallo poistetaan (=tuhotaan pöydält?.
  Help = 'direction.html';
type
...
var
  ...
  NumBalls:integer;          // Pallojen lukumäär?lomakkeen
                             // alustuksen jälkeen
  NumBallsDestroy:integer;   // Pöydölt?poistettujen pallojen lkm
...
begin
  ...
  begin
    ...
    pallo[i].Move;
    CollisionResponseBalls;   // Testataan onko törmäyksi?muihin palloihin
    CollisionResponseSides(param); // Testataan onko törmäyksi?laitoihin
...
// laitoihin törmäyksess?on oletuksena
// ett?pallon energia säilyy törmäyksess?
// muuttumattomana.
// Jos pallo menee pussiin tuhotaan pallo ja laitetaan se
// näytille ruudun ylälaitaan luomalla ilmentym?ShootBall- luokalle.
// Jotta saadaan jotain käyttöä sinäns?turhalle luokalle.
...
begin
    ...
    begin
      ...
      // Peruutetaan palloa kunnes se on pelialueen laitojen sisäll?
      begin
        BackUp(pm);
      end;
...
var
  ...
  radius:real;     // Lyöntipallon ja hiirell?osoitetun
                   // kohteen välinen ero
...
procedure TMainForm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_RETURN then Speed := Speed+1; // Lisää lyöntivoimaa yhdell?
  if Key=VK_SHIFT then Speed := Speed-1; // vähentää lyöntivoimaa yhdell?
...
begin
  Print(0,0,'Lyöntivoima Return+, Shift-. Tähtäys ja lyönti hiirell? Lyöntivoima: '+IntToStr(Speed));
...
begin
    ...
    begin
      NumBallsDestroy := NumBallsDestroy + 1; // Lisätään yhdell?poistettujen
                                              // pallojen lukumäärää.
      NumBalls := NumBalls -1;                // Vähennetään yhdell?pöydäll?
...
begin
  Speed := Speed-1; // vähentää lyöntivoimaa yhdell?
  if Speed < 0 then speed := 0;
  if Speed > MaxSpeed then speed := MaxSpeed;
end;
...
begin
  Speed := Speed+1; // lisää lyöntivoimaa yhdell?
  if Speed < 0 then speed := 0;
  if Speed > MaxSpeed then speed := MaxSpeed;
end;
